...
const validate = (modelName) => (req, res, next) => {
  ...
  for (const [field, opts] of Object.entries(def.fields)) {
    ...
    if (opts.type === 'email'  && !emailRegex.test(value))    errors.push(`${field} must be a valid email address.`);
    if (opts.type === 'number' && isNaN(Number(value)))        errors.push(`${field} must be a number.`);
    if (opts.type === 'date'   && isNaN(Date.parse(value)))    errors.push(`${field} must be a valid date.`);
